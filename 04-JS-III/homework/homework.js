...
function agregarNumeros(numeros) {
  ...
  for (let i = 0; i < numeros.length; i++)
    total = numeros[i] == Math.trunc(numeros[i]) ? total + numeros[i] : total;
  return total;
}
...
function mesesDelAño(array) {
  ...
  // Tu código:
  let narry = array.filter(e => (e == "Enero" || e == "Marzo" || e == "Noviembre"));
  return narry.length == 3 ? narry : "No se encontraron los meses pedidos";
}
...
function breakStatement(numero) {
  ...
  for (let i = 0; i < 10; i++) {
    if (numero == i) {
      ar = "Se interrumpió la ejecución";
      break;
    }
    numero += 2;
    ar.push(numero);
